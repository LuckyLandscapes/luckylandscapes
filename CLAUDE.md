...
The public website's project gallery is sourced from luckyapp, not the static `source-images/` pipeline. Two surfaces:
...
**Marketing site integration:** [`marketing/main.js`](marketing/main.js) `loadMarketingGalleryFromLuckyapp()` (search for `MARKETING_GALLERY_URL`) fires on page load. The hardcoded `projectData` array at the top of main.js is now `let` (was `const`) and serves as the FALLBACK shown if luckyapp is unreachable. On successful fetch, `projectData` is swapped + `homepageFeatured` is reset to `[0..min(6, N-1)]` + both `buildGalleryGrid()` and `buildCollectionGrid()` are re-invoked. Event delegation on the grid containers survives the rebuild because listeners are attached to the parent, not individual cards. **Don't delete the static fallback** — site degrades gracefully during luckyapp outages.
