...
- Latest migrations on disk: `029_job_workday_set.sql`, then the catalog rebuild trio — `030_suppliers_table.sql`, `031_materials_rebuild.sql` (DROPS and recreates materials with supplier_id FK + tax_rate + customer-visible flag), and `032_selected_materials.sql` (adds JSONB columns to quotes + contracts). `028` was deliberately skipped. Next one should be `033_…`.
- `FULL_REBUILD.sql` is **deprecated** — it only covers 001–013 and is missing both 14 newer tables (payments, contracts, time_segments, contractors, etc.) and the entire 030–032 catalog rebuild. Always run the numbered files in order; do not use the rebuild script.
