...
**Gallery page** at [`marketing/gallery.html`](marketing/gallery.html) renders a filter pill bar (`#gallery-filter-pills`) auto-populated from the tags present in `projectData`. Click handlers use event delegation on the parent container, so pills rebuild safely when remote data merges in. `prettyTitle()` in main.js hides generic titles like "Untitled 3" / "IMG_1234" so the public site never shows them — instead the tag chip carries the card. `onerror` on each `<img>` swaps in `/images/banner.jpg` with a desaturated filter if a Supabase URL ever fails to load.
