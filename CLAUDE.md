...
### Historical backfill — Google Sheets → Lucky (jobs + expenses CSV)
For migrating from spreadsheet tracking, `/jobs` has an **Import History** button that opens [`ImportHistoricalJobsModal`](luckyapp/src/components/ImportHistoricalJobsModal.js), and `/finance` has an **Import History** button that opens [`ImportHistoricalExpensesModal`](luckyapp/src/components/ImportHistoricalExpensesModal.js). Both share the two-step paste/upload → validate → apply flow from `ImportMaterialsModal`. Pure parsing + validation lives in [`src/lib/csvHistoricalImport.js`](luckyapp/src/lib/csvHistoricalImport.js); apply step is `bulkImportHistoricalJobs` / `bulkImportHistoricalCompanyExpenses` on the data layer.
