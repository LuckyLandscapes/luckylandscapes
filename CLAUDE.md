...
- **luckyapp control plane** at [`(dashboard)/marketing-gallery/page.js`](luckyapp/src/app/(dashboard)/marketing-gallery/page.js). Drag-drop multi-file upload, per-file title/description/tags (curated set in `GALLERY_TAGS`), optional before/after pair, publish toggle, drag-to-reorder via sort_order, edit/delete. Uses the shared [`compressImage`](luckyapp/src/lib/compressImage.js) helper (~300KB per photo). Storage path `<orgId>/<uuid>.<ext>` in the public `marketing-gallery` bucket. Linked from the Sidebar under Tools → "Website Gallery" (owner/admin only). The upload modal detects generic filenames ("IMG_1234", "Untitled 3") and refuses to submit until the title is meaningful — `looksGenericTitle()` is the single source of truth for this check, mirrored on the public site so legacy generic titles render as just the tag chip.
- **AI auto-fill endpoint** at [`/api/marketing/gallery/suggest`](luckyapp/src/app/api/marketing/gallery/suggest/route.js). POST with `Authorization: Bearer <supabase access_token>` + body `{ imageBase64, model? }`. Returns `{ title, description, tags, provider, usage }` strict JSON; tags validated against `GALLERY_TAGS`. Provider auto-selected by env var (Gemini wins when both set):
   - **`GEMINI_API_KEY`** (preferred, has free tier) — Google Gemini via `generativelanguage.googleapis.com`. Get a key in 30s at [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey). `responseMimeType: 'application/json'` forces strict JSON output. Three model tiers selectable from the upload modal's "AI model" dropdown (preference persists per-user in `localStorage` under `lucky_gallery_ai_model`):
     - `gemini-3.1-flash-lite` (DEFAULT) — cheapest, $0.25/$1.50 per MTok, fast and accurate enough for captions
     - `gemini-3-flash-preview` — smarter writing, $0.50/$3.00 per MTok
     - `gemini-3.1-pro-preview` — SOTA reasoning, $2.00/$12.00 per MTok (~$0.005/photo at typical sizes)
     - **Server validates the requested model against `GEMINI_ALLOWED`** so the client can't burn credit on arbitrary models. Auto-falls back to `gemini-2.5-flash` then `gemini-flash-latest` on 404 (covers Google deprecating a model mid-session — happened to 2.0 Flash on 2026-05). Other errors (400/429/etc.) abort instead of silently upgrading the tier.
     - To add a newer model: append to `GEMINI_ALLOWED` (server) and `GEMINI_MODEL_OPTIONS` (client), both at the top of their files.
