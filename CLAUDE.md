...
- **Gallery photos via luckyapp (primary path):** Riley uploads project photos through luckyapp's `/marketing-gallery` page from his phone in the field — auto-compressed to ~300KB, tagged, optional before/after. The marketing site's [`main.js`](marketing/main.js) `loadMarketingGalleryFromLuckyapp()` (search for `MARKETING_GALLERY_URL`) fetches `https://app.luckylandscapes.com/api/marketing/gallery` on page load and swaps the result into the in-memory `projectData` array, then re-renders the homepage gallery grid + the `/gallery` page collections. The static `projectData` array at the top of `main.js` is the FALLBACK shown only if luckyapp is unreachable. Updates go live in ~60s (Vercel edge cache TTL). **Don't delete the static fallback** — it keeps the site working during luckyapp outages. See "Marketing gallery (luckyapp-managed)" below for the full data layer.
...
- **Legacy-portfolio import** at [`/api/marketing/gallery/import-legacy`](luckyapp/src/app/api/marketing/gallery/import-legacy/route.js). One-time POST (auth-required, owner/admin only). Server-side: fetches each of the original static `projectData` images from `https://luckylandscapes.com/images/...`, re-uploads to the `marketing-gallery` Storage bucket under `<orgId>/legacy/<projSlug>/<photoSlug>.<ext>`, and inserts one `marketing_gallery` row per image (multi-photo projects split into individual cards — a 6-photo "Lawn Maintenance" project becomes 6 rows). Before/after pairs stay as one row with both URLs. **Idempotent** — uses the deterministic storage key as a uniqueness check; re-running skips existing rows. Concurrency=4 to stay within Vercel's function timeout. Triggered from the manage view's "Import legacy" button. **After import + verification: remove the `let projectData = [...]` array from [`marketing/main.js`](marketing/main.js)** so the site reads exclusively from the API; the fallback then becomes an empty array and the gallery just shows nothing during rare luckyapp outages instead of showing stale duplicates of imported entries.

**Marketing site integration:** [`marketing/main.js`](marketing/main.js) `loadMarketingGalleryFromLuckyapp()` (search for `MARKETING_GALLERY_URL`) fires on page load. The hardcoded `projectData` array at the top of main.js is now `let` (was `const`) and serves as the seed/fallback. On successful fetch, remote items are MERGED in front of the static portfolio (`projectData = [...converted, ...staticTagged]`) — visitors see Riley's latest work first, then the curated original portfolio. `staticProjectDataBackup` is snapshotted at module load so re-runs always merge against the original baseline. `homepageFeatured` is reset to the first 6 across both. Event delegation on the grid containers survives the rebuild. **Don't delete the static fallback** — site degrades gracefully during luckyapp outages AND keeps the gallery rich while Riley's still uploading.
