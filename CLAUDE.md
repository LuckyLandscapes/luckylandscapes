...
- **AI auto-fill endpoint** at [`/api/marketing/gallery/suggest`](luckyapp/src/app/api/marketing/gallery/suggest/route.js). POST with `Authorization: Bearer <supabase access_token>` + body `{ imageBase64 }`. Calls Claude Haiku 4.5 (`claude-haiku-4-5-20251001`) via native fetch (no SDK dep) with a constrained prompt that returns `{ title, description, tags }` strict JSON. Tags are validated against the same `GALLERY_TAGS` set. Cost: ~$0.001–0.003 per photo; 100 photos ≈ $0.15. **Requires `ANTHROPIC_API_KEY` in Vercel env vars** — without it the endpoint returns 503 and the upload modal hides the "Auto-fill" buttons gracefully (feature degrades, doesn't crash). Auth check exists so a leaked public URL can't burn org credit.
