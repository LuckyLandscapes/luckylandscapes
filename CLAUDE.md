...
- **AI auto-fill endpoint** at [`/api/marketing/gallery/suggest`](luckyapp/src/app/api/marketing/gallery/suggest/route.js). POST with `Authorization: Bearer <supabase access_token>` + body `{ imageBase64 }`. Returns `{ title, description, tags, provider, usage }` strict JSON; tags validated against `GALLERY_TAGS`. Supports two providers, auto-selected by env var (Gemini wins when both are set because free tier covers Lucky's volume):
   - **`GEMINI_API_KEY`** (preferred, FREE) — Google Gemini 2.0 Flash via `generativelanguage.googleapis.com`. Free tier: 1500 req/day + 15 req/min. Get a key in 30s at [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey). `responseMimeType: 'application/json'` forces strict JSON output (no markdown wrapping to parse around). Plenty for Riley's 10-20 photos/week.
   - **`ANTHROPIC_API_KEY`** (paid fallback) — Claude Haiku 4.5 (`claude-haiku-4-5-20251001`) via `api.anthropic.com`. ~$0.001-0.003/photo (100 photos ≈ $0.15). **Claude Max subscription doesn't grant API access** — that's separate billing. New API accounts get $5 free credit which covers ~3000 Haiku photo analyses.
   - Without either key the endpoint returns 503 and the upload modal hides the "Auto-fill" buttons gracefully (feature degrades, doesn't crash). Auth check exists so a leaked public URL can't burn org credit.
   - Both providers use native `fetch` — no SDK dependency. To swap to Sonnet 4.6 for higher-quality captions, change the `model` string inside `suggestViaAnthropic`. To swap to Gemini 2.5 Pro, change the URL path inside `suggestViaGemini`.
