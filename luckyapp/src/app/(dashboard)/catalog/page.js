            ...
            <div className="modal-body">
              <div style={{ display: 'flex', gap: 'var(--space-md)', marginBottom: 'var(--space-md)' }}>
                <div style={{ flex: 1, padding: 'var(--space-md)', background: 'rgba(34,197,94,0.10)', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#4ade80' }}>{starterResult.inserted}</div>
                ...
                <div style={{ flex: 1, padding: 'var(--space-md)', background: 'rgba(59,130,246,0.10)', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#60a5fa' }}>{starterResult.updated}</div>
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>Updated</div>
                </div>
                <div style={{ flex: 1, padding: 'var(--space-md)', background: starterResult.errors.length ? 'rgba(239,68,68,0.10)' : 'rgba(148,163,184,0.10)', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: starterResult.errors.length ? '#f87171' : 'var(--text-tertiary)' }}>{starterResult.errors.length}</div>
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>Errors</div>
                </div>
              </div>
              {starterResult.errors.length > 0 && (
                <details>
                  ...
                  <ul style={{ fontSize: '0.78rem', color: 'var(--text-tertiary)', marginTop: 8, maxHeight: 220, overflow: 'auto' }}>
                    {starterResult.errors.map((e, i) => <li key={i}><strong>{e.item}:</strong> {e.error}</li>)}
                  </ul>
                </details>
              )}
              {starterResult.inserted > 0 && (
                <div className="alert alert-info" style={{ marginTop: 'var(--space-md)' }}>
                  <strong>Next:</strong> click <strong>Refresh prices</strong> in the header to pull live prices and stock from outdoorsolutions-lincoln.com.
                </div>
              )}
            </div>
            <div className="modal-footer">
